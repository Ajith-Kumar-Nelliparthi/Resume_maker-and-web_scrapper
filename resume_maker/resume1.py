...
def save_rewritten_resume_pdf(content, output_path="rewritten_resume.pdf"):
    try:
        ...
        lines = content.split("\n")
        line_count = 0
        max_lines_per_page = 45  # Adjusted to fit one page
        ...
        for line in lines:
            ...
            if line_count >= max_lines_per_page and pdf.page_no() == 1:
                pdf.add_page()
                line_count = 0  # Reset count on new page
            if not line:
                pdf.ln(1)
                line_count += 1
                ...
                line_count += min(2, len(line) // 90 + 1)  # Cap at 2 lines per bullet
            else:
                pdf.multi_cell(0, 6, line)
                line_count += min(2, len(line) // 90 + 1)  # Cap at 2 lines for wrapped text
