...
# Check dependencies
required_libs = ['pdfplumber', 'fpdf', 'docx', 'sklearn', 'groq', 'dotenv', 'nltk', 'spacy']
...
def save_rewritten_resume_pdf(content, output_path="rewritten_resume.pdf"):
    try:
        ...
        # Common resume headers for fallback detection
        header_keywords = {"executive summary", "summary", "work experience", "education", "projects", "technical skills", "soft skills"}
        for line in lines:
            line = line.strip()
            line = link_pattern.sub(r'\1', line)
            if line_count >= max_lines_per_page and pdf.page_no() == 1:
                pdf.add_page()
            if not line:
                pdf.ln(3)
            elif (line.startswith("**") and line.endswith("**")) or line.lower() in header_keywords:
                pdf.set_font("Arial", "B", 12)  # Bold, larger headers
            ...
            line_count += (len(line) // 180 + 1)
        pdf.output(output_path)
        print(f"✅ Rewritten resume saved as PDF: {output_path}")
    except Exception as e:
        print(f"Error saving resume as PDF: {e}")
